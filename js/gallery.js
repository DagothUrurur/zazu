$(document).ready(function () {
  ...
  function updateContestTimer() {
    ...
    if (contestStart && contestEnd) {
      if (now < contestStart) {
        ...
        desktopTimer.html(
          `<i class="fas fa-hourglass-start"></i> До начала: <span class="time-remaining">${timeStr}</span>`
        );
        mobileTimer.html(
          `<i class="fas fa-hourglass-start"></i> Начало через: ${formatMobileTime(
            diff
          )}`
        );
        ...
        desktopTimer.html(
          `<i class="fas fa-hourglass-half"></i> До конца: <span class="time-remaining">${timeStr}</span>`
        );
        mobileTimer.html(
          `<i class="fas fa-hourglass-half"></i> Осталось: ${formatMobileTime(
            diff
          )}`
        );
  ...
  $("#artworkModal").on("show.bs.modal", function (event) {
    ...
    $.get(
      "php/get_artwork.php?id=" + artworkId,
      function (artwork) {
        // Обновляем основную информацию
        modal
          .find("#artworkModalTitle")
          .text(artwork.title + " @" + artwork.author);
        modal
          .find("#modalArtworkImage")
          .attr("src", "/uploads/artworks/" + artwork.image_path);
        ...
        if (isContest) {
          ...
          updateVoteButton(
            voteBtn,
            artwork.votes_count || 0,
            artwork.user_has_voted
          );
    ...
    $.post(
      ...
      function (response) {
        if (response.status === "success") {
          ...
          btn
            .toggleClass("voted", response.action === "added")
            .find("i")
